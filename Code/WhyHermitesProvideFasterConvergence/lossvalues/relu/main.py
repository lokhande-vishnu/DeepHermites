...
def train(epoch, current_lr, train_grad_eta):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        eta_ls = [0, 0.00005, 0.0001, 0.0005, 0.001, 0.005, 0.01]
        dgrad_eta = []
        g_net = []
        for (n, p) in net.named_parameters():
            if p.grad is not None:
                g_net.extend(p.grad.flatten().tolist())
        for i in range(len(eta_ls)-1):
            ...
            for (nc, pc) in net_copy.named_parameters():
                if pc.grad is not None:
                    g_net_copy.extend(pc.grad.flatten().tolist())
            dgrad_eta.append(torch.norm(torch.tensor(g_net)-torch.tensor(g_net_copy)).item())
        ...
        if (batch_idx+1) %100 == 0:
            ...
            with open("train_grad_eta_relu.csv", "w+") as f:
                writer = csv.writer(f)
