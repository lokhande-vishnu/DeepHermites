...
class SmallNORB(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        target = self.labels[
            index % 24300] if self.mode is "all" else self.labels[index]
        if self.target_transform is not None:
            target = self.target_transform(target)

        info = self.infos[index %
                          24300] if self.mode is "all" else self.infos[index]
    ...
    def _load(self, file_name):
        return torch.load(
            os.path.join(self.root, self.processed_folder,
                         f"{file_name}{self.extension}"))

    def _save(self, file, file_name):
        with open(
                os.path.join(self.root, self.processed_folder,
                             f"{file_name}{self.extension}"), 'wb') as f:
            torch.save(file, f)

    def _check_exists(self):
        """ Check if processed files exists."""
        files = (f"{self.train_image_file}_left",
                 f"{self.train_image_file}_right",
                 f"{self.test_image_file}_left",
                 f"{self.test_image_file}_right", self.test_label_file,
                 self.train_label_file)
        fpaths = [
            os.path.exists(
                os.path.join(self.root, self.processed_folder,
                             f"{f}{self.extension}")) for f in files
        ]
    ...
    def download(self):
        ...
        test_label = self._read_info_file(
            self.data_files["test"]["info"]["name"])
        test_info = self._read_label_file(
    ...
    def _parse_header(file_pointer):
        ...
        # Read dimensions
        dimensions = []
        num_dims, = struct.unpack(
            '<i', file_pointer.read(4))  # '<' is little endian)
    ...
    def _read_image_file(self, file_name):
        ...
        with open(fpath, mode='rb') as f:
            ...
            left_samples = np.zeros(
                shape=(num_samples, height, width), dtype=np.uint8)
            right_samples = np.zeros(
                shape=(num_samples, height, width), dtype=np.uint8)
    ...
    def _read_image(file_pointer, height, width):
        ...
        image = struct.unpack('<' + height * width * 'B',
                              file_pointer.read(height * width))
